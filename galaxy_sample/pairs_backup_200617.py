...
class pair_list:
    ...
    def from_grid(cls,grid,geometry,dtype='int64',parent=None):
        ...
        for chunk in grid.iter_pairs(chunk=True):
            ...
            if geometry=='regular':
                #TO BE CHECKED
                pair_chunk.separations = np.sqrt(np.sum( (grid.points[pairs[:,0]]-grid.points[pairs[:,1]])**2,
                                                 axis=1 ))
                pair_chunk.separations = np.sqrt(np.sum( np.diff(grid.points[pair_chunk.pairs],axis=1)**2, axis=1 ))
...
class corrfunc:   
    ...
    def plot(self,ax,mode,*args,**kwargs):
        """
