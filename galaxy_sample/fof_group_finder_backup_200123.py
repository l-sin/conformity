...
class fof_group_finder:
    def __init__(self,sample,sky_fraction, az, pol, redshift, stellarmass, spacing):        
        self.sample = sample
...
def YangCurves(sample, true_group_ids, obs_group_ids, thresholds, bin_edges):
    ...
    MatchMvir, completeness, contamination, purity = YangEvaluate(sample, true_group_ids, obs_group_ids)
    
    labels = zip( bin_edges[:-1], bin_edges[1:] )
    BM = list(chain(  mf.binmembers(np.array(MatchMvir),bin_edges)  ))
    
    curve = lambda stat,members: np.array([np.mean(np.array(stat)[np.array(members)]>t) for t in thresholds])
