...
class tinker_group_finder(fof_group_finder):
    ...
    def run(self, B, rscale_mod=1,vel_disp_mod=1, max_iter=30, masslimited=False):
        ...
        while not all(Converged) and N_iter<max_iter:
            # On first iteration, initialize haloes via SHAM. Thereafter, compute halo properties via normal AM
            if N_iter==0:
                AM = AMproto(src=src,SHAM=True)
            ...
            print(MyEvaluate(self.sample, self.sample.data['FOFCentralGal'], groupIDs))
