...
class fof_group_finder:
    ...
    def link_pairs(self,l_p,l_z,density='redshift'):
        ...
        if max(l_p*L)>max(self.pairs.separations) or max(l_z*L)>max(self.pairs.dR_comoving):
            warn( 'Separations in precomputed list of pairs ({},{}) shorter than linking lengths ({},{})'.format(
            max(self.pairs.separations),max(self.pairs.dR_comoving),max(l_p*L),max(l_z*L) ) )
...
def MyEvaluate(sample,true_group_ids, obs_group_ids):
    P = sample.pair_list
    cdz = 299792.458*np.abs(np.diff( sample.data['redshift'][P.pairs], axis=1)).reshape(-1)
    P = P.select(np.logical_and( P.separations<1, cdz<500))
...
def GlobalCompleteness(sample, true_group_ids, obs_group_ids):
    #MatchMvir, completeness, contamination, purity = [], [], [], []
    f_halo = dict()
    ...
    for GrNr,group in TrueGroups.items():
        if len(group)>=1:
            maxmass = np.argmax(sample.data['StellarMass'][group])
            maxmassidx = group[maxmass]
            
            ObsGroupNr = obs_group_ids[maxmassidx]
            ObsGroupMaxMass = np.argmax(sample.data['StellarMass'][ObsGroups[ObsGroupNr]])
            ObsGroupMaxMassIdx = ObsGroups[ObsGroupNr][ObsGroupMaxMass]
            
            TrueMh = sample.data['CentralMvir'][group]
            
            if not all(TrueMh[0]==TrueMh):
                raise Exception('Funky')
            
            if maxmassidx==ObsGroupMaxMassIdx:
                f_halo[TrueMh[0]]=1
            else:
                f_halo[TrueMh[0]]=0
    return f_halo